...
from lvkit.project_store import find_project_store, init_project_store
...
def test_find_project_store_in_cwd(tmp_path: Path) -> None:
    """find_project_store returns .lvkit/ if directly under start dir."""
    (tmp_path / ".lvkit").mkdir()
    assert find_project_store(start=tmp_path) == tmp_path / ".lvkit"


def test_find_project_store_walks_up(tmp_path: Path) -> None:
    """Walks up the directory tree looking for .lvkit/."""
    (tmp_path / ".lvkit").mkdir()
    deep = tmp_path / "src" / "subpkg"
    deep.mkdir(parents=True)
    assert find_project_store(start=deep) == tmp_path / ".lvkit"


def test_find_project_store_stops_at_git(tmp_path: Path) -> None:
    """Stops walking at a .git directory marker."""
    # Outer dir has .lvkit/, but inner dir has .git/ — should NOT find outer .lvkit/
    (tmp_path / ".lvkit").mkdir()
...
def test_find_project_store_returns_none_when_absent(tmp_path: Path) -> None:
    """Returns None when no .lvkit/ exists anywhere on the path."""
    deep = tmp_path / "a" / "b" / "c"
    deep.mkdir(parents=True)
    # Drop a .git so we don't escape into a real parent .lvkit/
    (tmp_path / ".git").mkdir()
    assert find_project_store(start=deep) is None


def test_init_project_store_creates_layout(tmp_path: Path) -> None:
    """init_project_store creates .lvkit/ with README and category dirs."""
    store = init_project_store(tmp_path)
    assert store == tmp_path / ".lvkit"
    assert store.is_dir()
...
# ============================================================
# Regression: absent .lvkit/ behaves identically
# ============================================================
...
def test_install_claude_skills_refuses_local_edits(tmp_path: Path) -> None:
    ...
    install_claude_skills(tmp_path)
    edited = tmp_path / ".claude" / "skills" / "lvkit-convert" / "SKILL.md"
...
def test_install_claude_skills_force_overwrites(tmp_path: Path) -> None:
    ...
    assert "name: lvkit-convert" in edited.read_text()
...
def test_install_claude_skills_atomic_on_conflict(tmp_path: Path) -> None:
    ...
    (skills_dir / "lvkit-convert" / "SKILL.md").write_text("LOCAL EDIT\n")
    ...
    assert (skills_dir / "lvkit-convert" / "SKILL.md").read_text() == "LOCAL EDIT\n"
...
def test_install_copilot_skills_prompt_bodies_use_lvkit_prefix(
    tmp_path: Path,
) -> None:
    """Prompt bodies must reference other skills with lvkit- prefix.
    ...
    for skill in _LVKIT_SKILLS:
        ...
        # as bash command flags (those should be `lvkit describe` —
...
def test_install_copilot_skills_atomic_on_conflict(tmp_path: Path) -> None:
    ...
    (prompts_dir / "lvkit-convert.prompt.md").write_text("LOCAL EDIT\n")
...
def test_install_copilot_skills_force_overwrites(tmp_path: Path) -> None:
    ...
    install_copilot_skills(tmp_path)
    edited = tmp_path / ".github" / "prompts" / "lvkit-convert.prompt.md"
    edited.write_text("LOCAL EDIT\n")
...
def test_mcp_discover_from_vi_file(tmp_path: Path) -> None:
    """MCP helper finds .lvkit/ when given a file path inside the project."""
    from lvkit.mcp.server import _configure_resolvers_for_vi
...
def test_mcp_discover_from_directory_path(tmp_path: Path) -> None:
    ...
    .lvkit/ when it lived inside the directory.

    Setup: the .lvkit/ store lives INSIDE the lvlib directory itself, with
    ...
    `lvlib_dir` itself, so it would miss `lvlib_dir/.lvkit/`. We verify the
    ...
    # .lvkit/ inside the lvlib — buggy code (start from lvlib.parent)
    # would walk tmp_path (no .lvkit) → .git → return None.
    ...
    try:
        ...
        assert entry["name"] == "MCP-DIR-TEST OVERRIDE", (
            "MCP did not load .lvkit/ from inside the directory path — "
...
def test_mcp_discover_returns_none_outside_project(tmp_path: Path) -> None:
    ...
    # Drop a .git so discovery stops here without finding any .lvkit/
    (tmp_path / ".git").mkdir()
