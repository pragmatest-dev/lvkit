...
def mark(c):
    ...
    return (
        f'<path d="M15 32 H32 M32 32 V13 H49 M32 32 V51 H49" {STROKE.format(c=c)}/>'
        + TERM.format(x=5, y=27, c=c)
        + TERM.format(x=49, y=8, c=c)
        + TERM.format(x=49, y=46, c=c)
        + JUNC.format(c=c)
    )
...
cairosvg.svg2png(
    bytestring=msvg.encode(),
    output_width=256,
    write_to=str(HERE / f"{SLUG}-mark-256.png"),
)
for sz in (128, 256):
    cairosvg.svg2png(
...
for sz in (16, 32, 48):
    cairosvg.svg2png(
        bytestring=msvg.encode(),
        output_width=sz,
        write_to=str(HERE / f"favicon-{sz}.png"),
    )
...
cairosvg.svg2png(
    bytestring=sd.encode(),
    output_width=pngw,
    write_to=str(HERE / f"{SLUG}-wordmark.png"),
)
