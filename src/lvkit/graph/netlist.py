...
# ``ambiguous_bares``/``instance_line``/``scope_header`` moved there too even
# though ``diff.py`` also imports and calls them: ``render_netlist``'s own
# call graph needs all three, and ``netlist_render_deprecated.py`` must never
# import from this module (that would be a fragile, import-order-dependent
# circular import). Re-exported here so every existing ``from
...
# ============================================================
#
# ``render_netlist`` itself and most of this section's original helpers
# (``ambiguous_bares``, ``instance_line``, ``scope_header``, and the whole
# ``_render_*``/``*_definition_line`` cluster) moved to
# ``netlist_render_deprecated.py`` and are re-exported above.  ``index_module``/
# ``component_line`` below are NOT part of that DEPRECATED renderer's own
# call graph -- they're used only by ``diff.py`` and the CLI's
# ``## Components`` table respectively -- so they stayed here.
