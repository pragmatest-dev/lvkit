---
name: lvpy-resolve-primitive
description: Resolve a single unknown LabVIEW primitive by following a strict verification process against documentation and graph context. Called when PrimitiveResolutionNeeded fires during lvpy generate.
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch
---

# Resolve Unknown Primitive

When `PrimitiveResolutionNeeded` fires for an unknown primitive during `lvpy generate`, this skill resolves it by identifying the function and writing a JSON mapping. Follow ALL steps IN ORDER. Do NOT skip. Do NOT guess.
...
Read `pyproject.toml` from the current directory (walk up if needed). If it contains `name = "lvpy"`, you are working **inside lvpy itself**:

- Destination: `data/primitives.json` (lvpy's shipped, cleanroom data)
- Cross-reference corpus: lvpy's `samples/` directory of real VIs
...
- Destination: `.lvpy/primitives.json` (project-local store; run `lvpy init` first if `.lvpy/` doesn't exist)
...
- Do NOT cross-reference lvpy's `samples/` — the user's project doesn't have them
- The mapping you write may be derived from licensed sources; that's the user's call. lvpy itself never reads `.lvpy/`.

In **both** modes, the function-identification step (Step 5) uses live web search against NI's online documentation. lvpy does not bundle the LabVIEW reference manual.
...
### lvpy mode: cross-reference lvpy's samples

Search for ALL instances across lvpy's sample corpus to see terminal variations:

```bash
python3 -c "
import sys; sys.path.insert(0, 'src')
from pathlib import Path
from lvpy.parser.vi import parse_vi
...
The fastest way to see context is `lvpy describe` on the calling VI:

```bash
lvpy describe "<path-to-calling-vi>" --search-path "<library-path>"
```
...
lvpy does not bundle NI's reference manual. Use the WebSearch tool to look up candidate functions on NI's documentation site.
...
- **lvpy mode**: cross-reference the `samples/` corpus more aggressively (Step 2), and ask the user
...
- **lvpy mode** → `data/primitives.json`
- **user-project mode** → `.lvpy/primitives.json`
...
- NEVER write user-mode mappings into lvpy's `data/` (cleanroom contamination)
- NEVER write lvpy-mode mappings into a user project's `.lvpy/` (wrong destination)
