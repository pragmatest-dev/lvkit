...
lvpy ships as cleanroom — its bundled data directory contains only
mappings derived from public documentation. A project-local `.lvpy/`
directory lets users with LabVIEW licenses contribute their own mappings
(potentially derived from licensed sources) without contaminating lvpy's
shipment.

The store is a parallel layout to lvpy's bundled data:
...
Resolvers load `.lvpy/` first, then fall back to lvpy's bundled data.

This module also installs Claude Code skills and Copilot instructions
from the packaged templates so downstream users can run lvpy's resolve
...
PROJECT_STORE_DIR = ".lvpy"

_README_TEMPLATE = """# .lvpy/ — Project-local resolution store

This directory is your project's private mapping store for lvpy. lvpy ships
...
sources, or third-party libraries you have rights to use. lvpy reads
`.lvpy/` **first** and only falls back to its shipped `data/` if no project
mapping exists.

## License boundary

**lvpy itself never reads `.lvpy/` into its own data files.** Anything you
put here stays in your project. Do not submit content from `.lvpy/` upstream
to lvpy unless it is independently cleanroom-derived.

If any file under `.lvpy/` is derived from licensed material (vi.lib block
...
The format mirrors lvpy's shipped `data/` exactly. Copy an example from
`<lvpy install>/data/` if you need a starting point.

## Populating it

When `lvpy generate` hits an unknown primitive or vi.lib VI, it raises a
...
If you use Claude Code, run `lvpy init --skills claude` to install
resolution skills into your project's `.claude/skills/`. For Copilot or
Cursor, use `lvpy init --skills copilot`.
"""


def find_project_store(start: Path | None = None) -> Path | None:
    """Walk up from `start` (default CWD) looking for a .lvpy/ directory.

    Stops at the first match, at a `.git` directory marker, or at the
    filesystem root. Returns the path to the .lvpy/ directory, or None if
    ...
    Returns:
        Path to the .lvpy/ directory, or None.
    """
    ...
    for candidate in [current, *current.parents]:
        ...
        if store.is_dir():
            return store
        # Stop at repo root — don't escape into a parent project's .lvpy/
...
def init_project_store(root: Path) -> Path:
    """Create .lvpy/ under `root` with a template README.md.

    Idempotent: if .lvpy/ already exists, leaves it alone but ensures the
    README is present (does not overwrite an existing README).

    Args:
        root: Directory under which to create .lvpy/.

    Returns:
        Path to the created (or existing) .lvpy/ directory.
    """
...
# ============================================================
#
# lvpy ships user-facing skill templates as package data under
# `src/lvpy/skill_templates/lvpy-<name>/SKILL.md`. The functions below
# install them into a downstream user's project so the user's LLM
# editor (Claude Code, Copilot, Cursor) can run lvpy's workflows.
#
# Two install targets:
#
...
#   .github/instructions/lvpy.instructions.md router that auto-loads
...
# Marker comment lvpy embeds in every generated Copilot file so users
# (and re-installs) can recognize a lvpy-managed file.
_COPILOT_MANAGED_MARKER = (
    "<!-- lvpy: managed by `lvpy init --skills`, "
...
def _iter_skill_templates() -> list[Any]:
    ...
    """
    template_root = files("lvpy.skill_templates")
    return sorted(
        (
...
def install_claude_skills(target_dir: Path, force: bool = False) -> list[Path]:
    """Install lvpy's Claude Code skills into a project.

    Copies every packaged template under
    `src/lvpy/skill_templates/<name>/SKILL.md` into
...
def install_copilot_skills(
    ...
    """Install lvpy's workflows for GitHub Copilot.
    ...
    - `instructions/lvpy.instructions.md` — single router file. Auto-
    ...
    router_dest = target_dir / ".github" / "instructions" / "lvpy.instructions.md"
...
def _yaml_quote(value: str) -> str:
    ...
    Minimal — handles the only YAML escapes lvpy needs (`"` and `\\`).
    """
    return '"' + value.replace("\\", "\\\\").replace('"', '\\"') + '"'


def _build_copilot_router(skill_dirs: list[Any]) -> str:
    """Build the .github/instructions/lvpy.instructions.md router.
    ...
    lines = [
        ...
        _COPILOT_MANAGED_MARKER,
        "",
        "# lvpy: LabVIEW VI to Python workflows",
        "",
        "This project uses lvpy for LabVIEW VI to Python conversion.",
    ...
    lines.extend(
        [
            "",
            "Run `lvpy init --skills copilot` to refresh these prompts and",
            "this router file.",
            "",
        ]
    )
...
def _yaml_get(frontmatter: str, key: str) -> str | None:
    ...
    Intentionally minimal — lvpy's SKILL.md frontmatter only uses flat
