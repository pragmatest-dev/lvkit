...
class VILibResolutionNeeded(Exception):
    ...
    def _format_message(self) -> str:
        ...
        msg += (
            "\nFix: add terminal info to .lvpy/vilib/<category>.json"
...
class VILibResolver:
    ...
    def __init__(
        ...
        Args:
            ...
            project_data_dir: Optional project-local .lvpy/ directory.
...
def reset_resolver(project_data_dir: Path | None = None) -> None:
    ...
    Call this at CLI/MCP entry points after discovering a project's .lvpy/
    directory. Subsequent get_resolver() calls return a resolver that loads
    project data first and falls back to shipped data.

    Args:
        project_data_dir: Path to project's .lvpy/ directory, or None to
