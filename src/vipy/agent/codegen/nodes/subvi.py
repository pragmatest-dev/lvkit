...
from vipy.graph_types import Operation, SubVIOperation, Terminal
...
def generate(node: SubVIOperation, ctx: CodeGenContext) -> CodeFragment:
    ...
    if vilib_vi and vilib_vi.python_code and vilib_vi.inline:
        return _generate_inline(node, ctx, vilib_vi)
    ...
    args, keywords = _build_arguments(node, ctx, vilib_vi)
    ...
    if keywords:
        ...
        call = ast.Call(
            func=ast.Name(id=func_name, ctx=ast.Load()),
            args=[_to_ast_value(a) for a in args],
            keywords=[],
        )
    ...
    bindings = _build_output_bindings(node, result_var, vilib_vi, ctx)
