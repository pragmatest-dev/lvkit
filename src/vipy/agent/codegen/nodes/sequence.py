...
class FlatSequenceCodeGen(NodeCodeGen):
    """Generate code for LabVIEW flat sequence structures.

    Flat sequences enforce execution order: frame 0 runs first,
    then frame 1, then frame 2, etc. In Python, this is just
    sequential code — no special control flow needed.
    ...
    def generate(self, node: Operation, ctx: CodeGenContext) -> CodeFragment:
        ...
        for frame in node.case_frames:
            ...
            # After each frame, propagate tunnel bindings for the next frame.
            # flatSeqTun tunnels chain values between frames (e.g. task ref).
            # Inner terminals bound by frame ops need to propagate to outer,
            # then outer→inner for the next frame's tunnels.
            for tunnel in node.tunnels:
