...
**Date the changelog BEFORE tagging — never after.** At the commit you tag, `CHANGELOG.md` must already have a dated `## [X.Y.Z] - <date>` section for the version (with a fresh empty `## [Unreleased]` reopened above it) — never the version's changes stranded under `[Unreleased]`. PyPI/the release consume the changelog and the published version is immutable, so dating it afterward does not fix the shipped release. Release order: (1) merge → (2) **date the changelog** → (3) bump the version sites (`pyproject` + `__init__` + `uv.lock` + `editors/vscode/package.json`) → (4) tag `vX.Y.Z` (PyPI) / `ext-vX.Y.Z` (bundles) → (5) **redeploy the Cloud Run render service** (`gcloud builds submit --config deploy/cloudrun/cloudbuild.yaml ...` from the repo root — see `deploy/cloudrun/README.md`). Step 5 is mandatory, not optional: the render service **builds lvkit from repo source, not PyPI**, so a release is not fully shipped until it is redeployed to the tagged source. Tagging and the Cloud Run deploy are each publishing — treat each as its own confirmed step.
