...
description: MANDATORY for ALL LabVIEW primitive (primResID) and terminal work in lvkit — invoke this BEFORE editing data/primitives.json or any vilib terminal index, and before reasoning about a terminal's role, parmIndex, or operand order. Triggers include: PrimitiveResolutionNeeded during lvkit generate; a render/describe showing unknown_primitive_N (#<id>); a request to identify, VERIFY, or CORRECT a primResID (a user saying "X should be Y", a suspected-wrong mapping, or a duplicate name in primitives.json); and resolving which parmIndex is which terminal (delimiter vs format, operand order, same-typed terminals). Enforces the no-guess process: identify by full connector-pane TYPES plus observed caller WIRING — never by terminal shape or by NI doc/reading order — one primitive at a time.
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch
---

# Resolve Unknown Primitive

**Invoke this skill for ALL `primitives.json` / terminal work — not only unknown primitives.** That means: an unknown primitive (`PrimitiveResolutionNeeded`), **verifying or correcting a suspected-wrong mapping** (a user reports "X should be Y"), and **resolving which `parmIndex` is which terminal role / operand order**. If you are about to hand-edit `primitives.json`, or you catch yourself inferring a terminal role from NI **doc/reading order**, STOP — that is this skill's job, and doc order does NOT map to `parmIndex` (roles come from observed `termBounds` geometry + the NI connector-pane image, Step 5c/6). This skill resolves it by identifying the function and writing a JSON mapping. Follow ALL steps IN ORDER. Do NOT skip. Do NOT guess.
...
- Terminal **indices** MUST be read from THIS VI's observed wiring/heap (Steps 2-3), NEVER assumed from documentation order — and NEVER copied from another VI. **There is no shared index rule, not even a reliable baseline.** A VI/subVI carries a connector-pane *pattern* (its `conId`, catalog in `data/connector_pane_patterns.json`) whose index origin + direction differ by pattern. A **PRIMITIVE is different and simpler: it has no conId and you never *derive* its index at all.** Every terminal's index is serialized **verbatim** as `parmIndex` inside that terminal's nested `<dco>` in the node's `<termList>` (the parser surfaces it as `terminal.index` — see Step 1's extraction), right next to its `termBounds` (pixel rectangle → position; sort inputs by `(top,left)`, top = operand 1) and its `typeDesc` (→ the type). So a primitive's index/position/type are **read straight from the BD XML `<dco>`**, never inferred from a pattern, a shape, or NI doc order. What you DO still have to work out is a terminal's *role* when two terminals share a type (e.g. delimiter vs format, both String) — that comes from the observed caller **wiring** (which control/constant feeds each `parmIndex`) + semantic coherence, not from geometry. Known-divergent examples: the common **4x2x2x4** pattern indexes Right→Left, Bottom→Top (idx 0 = bottom-right, outputs on the right get the LOW indices); but **5x2x2x2x5** (misnomer "5x3x3x3x5") starts idx 0 in the **top-left and increments the other way** — same-family patterns, opposite origins. Documentation lists terminals in reading order, which the pane indices do not follow (e.g. Split 1D Array docs list "array, index" but the pane has index=2 = numeric index, index=3 = array). So never infer an index from a pattern name or a "usual" rule — read the observed indices for this specific VI.
- Operand **order** for a non-commutative op (`a - b`, `a / b`, `a mod b`, the comparisons, quotient/remainder) comes from **physical `termBounds` geometry, NOT the index number** — the index is an unreliable proxy for position (see the pattern note above). LabVIEW's fixed convention: the **top input is operand 1** (ties broken left-first). So sort the input terminals by `termBounds` `(top, left)` and map the upper/left one to the first operand in the template. This is invariant per `primResID` (every instance of a primitive has the same pane), and it's exactly the check `scripts/audit_terminal_order.py` (AUDIT 1) runs — it's what distinguishes `in_1 - in_2` from `in_2 - in_1` and would have caught the historical Subtract inversion.
