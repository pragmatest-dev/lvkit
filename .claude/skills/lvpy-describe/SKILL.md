...
The `lvpy describe` CLI is the simplest entry point.
...
```bash
lvpy describe "<vi-path>" --search-path "<library-path>" --chart
```
...
from lvpy.graph.describe import describe_vi, describe_operations, describe_constants
...
from pathlib import Path
from lvpy.graph.core import InMemoryVIGraph
from lvpy.graph.describe import describe_vi
...
`lvpy describe` and the underlying graph functions never require resolution to succeed. Unknown primitives and vi.lib VIs render as `[prim N]` / their bare name — you can describe a VI even if lvpy has no mapping for some of its operations.
