---
name: lvpy-resolve-vilib
description: Resolve a single unknown vilib VI by looking up its terminal layout in the LabVIEW documentation. Called when VILibResolutionNeeded fires during lvpy generate.
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch
---

# Resolve Unknown vilib VI

When `VILibResolutionNeeded` fires during `lvpy generate`, this skill resolves it. vilib VIs have KNOWN names (they're filenames), so we can look them up directly. Follow ALL steps IN ORDER.
...
- Destination: `data/vilib/<category>.json` (lvpy's shipped, cleanroom data)
...
- Destination: `.lvpy/vilib/<category>.json` (project-local store; run `lvpy init` first if `.lvpy/` doesn't exist)
...
- The mapping you write may be derived from the actual vi.lib source; that's the user's call. lvpy itself never reads `.lvpy/`.
...
### lvpy mode: web search NI's documentation
...
If `lvpy describe` works on the file (it doesn't require resolution), use it for a quick terminal layout:

```bash
lvpy describe "<full-vilib-path>"
```

If the file isn't accessible, fall back to web search (same as lvpy mode above) or ask the user to open the VI in LabVIEW and read off the connector pane terminal positions. The connector pane index is what lvpy needs.
...
```bash
grep -r "VI NAME" .lvpy/vilib/ 2>/dev/null
...
- **lvpy mode** → `data/vilib/<category>.json`
- **user-project mode** → `.lvpy/vilib/<category>.json` (and add the category to `.lvpy/vilib/_index.json` if it's not there yet)
...
Rules:
- Terminal names come from the DOCUMENTATION (lvpy mode) or SOURCE FILE (user mode), not guesses
